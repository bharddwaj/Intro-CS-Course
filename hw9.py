...
def letterScoreLoop(letter, scorelist):
    ...
    for i in scorelist:
        if i[0] == letter:
            return i[1]
    return 'Assumed the letter is in scorelist so this statement should never occur'
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' Exercise 4
...
def wordScoreLoop(S, scorelist):
    ...
    scrabble_score = 0
    for i in S:
        scrabble_score += letterScoreLoop(i,scorelist)
...
def wordsWithScoreLoop(dct, scores):
    ...
    new_list = []
    for i in dct:
        new_list.append([i,wordScoreLoop(i, scrabbleScores)])
    return new_list
